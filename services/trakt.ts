...
router.post("/auth/refresh", async (ctx) => {
  ...
  try {
    ...
    logError("Token refresh failed for Trakt: ", error);
    ctx.response.status = 400;
    ctx.response.body = { error: "Token refresh failed" };
  }
});

// Example protected route (e.g., fetching user data)
router.get("/api/trakt/user", async (ctx) => {
  ...
  try {
    ...
    logError("Trakt API call failed: ", error);
    ctx.response.status = 500;
