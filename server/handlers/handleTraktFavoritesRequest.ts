...
import { getTraktFavorites, getRecentWatchedIds } from "../services/trakt.ts";
import { SEARCH_COUNT, GEMINI_API_KEY } from "../config/env.ts";
...
export const handleTraktFavoritesRequest = async (ctx: Context) => {
  const { tmdbKey, tmdbLanguage, traktKey, rpdbKey, omdbKey, userId, type, googleKey, traktCreateLists } = ctx.state;
  ...
  const cacheKey = `user:${userId}:favorites-${type}`;
  let metas: Meta[] = [];
  ...
  try {
    ...
    const recentWatches = await getTraktFavorites(type, traktKey, SEARCH_COUNT);
    ...
    if (metas.length > 0) {
      ...
      await Promise.all([
        ...
        traktCreateLists ? pushListToQstash(tasks) : Promise.resolve(null)
      ]);
    }
